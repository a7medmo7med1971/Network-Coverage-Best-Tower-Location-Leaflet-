 $(document).ready(function(){ //// لما الصفحة تحمل اعملي الكود
...
towerNet.eachLayer(function (layer) {
 ...
 map.removeLayer(towerNet);
  ...
  let wms= L.tileLayer.wms("http://10.100.100.106:8051/geoserver/gov/wms", {
...
L.control.layers(baseMaps, overlayMaps, {
  ...
  map.on("draw:edited",function(Kanba){
      ...
      layers.eachLayer(function(layer){ ///  btnSave in draw:edited
        alert("tam");
      })
  })
  //*********************************************************************************************************/
  ...
  let cairoLayer = L.geoJSON(cairo,{
    ...
    onEachFeature: function (feature, layer) {  // تفاعل مع الداتا وعرضها
  ...
  function createBuffer() {
    ...
    bufferss = turf.featureCollection(
      towerPoints.features.map(point =>
        turf.buffer(point, radius, { units: "kilometers" })      //3
      )
    );
  
    // عرض البافر على الخريطة
    bufferLayerr = L.geoJSON(bufferss, {                   //4
      style: {
        color: 'blue',
        fillColor: 'blue',
        fillOpacity: 0.1
  ...
  document.getElementById("radiusInputt").addEventListener("input", function () {
...
function suggestKMeansTowers(k = 4, bufferRadius = 0.5) {
  ...
  const centroids = turf.featureCollection(uncovered.map(f => turf.centroid(f)));
  ...
  const clusters = [...new Set(clustered.features.map(f => f.properties.cluster))];
  ...
  clusters.forEach(clusterId => {
    const points = clustered.features.filter(f => f.properties.cluster === clusterId);
    ...
    const leafletLatLng = [center.geometry.coordinates[1], center.geometry.coordinates[0]];
...
suggestTower.addEventListener("click", function () {
  ...
  const userRadius = parseFloat(document.getElementById('radiusInputt').value);
...
////////////////////////////////////////selectBuilding//////////////////////////////////////////////////////////////////
document.getElementById("selectBuilding").addEventListener("click", function () {
  ...
  cairoLayer.eachLayer(layer => {
    ...
    if ((intersectManual && intersectSuggested > 0) || intersectSuggested >= 2) {
      layer.setStyle({
        color: "blue",
        fillColor: "blue",
        fillOpacity: 0.7,
...
document.getElementById("downloadData").addEventListener("click", function () {
  ...
  const proposedMarkers = window._suggestedKmeansLayers?.filter(l => l instanceof L.Marker) || [];
  const proposedBuffers = window._suggestedKmeansLayers?.filter(l => l instanceof L.GeoJSON)
    .map(l => l.toGeoJSON().features[0]) || [];
  ...
  cairoLayer.eachLayer(layer => {
    ...
    let isManual = manualBuffers.some(buffer => turf.booleanContains(buffer, geom));
    let suggestedCount = proposedBuffers.filter(buffer => turf.booleanIntersects(buffer.geometry, geom)).length;
  ...
  const uncovered = totalBuildings - (coveredManual + coveredSuggested + blueCount);
...
document.getElementById("display").addEventListener("click", function () {
  ...
  cairoLayer.eachLayer(layer => {
    const geom = layer.feature.geometry;
    const isCovered = bufferFeatures.some(buffer => turf.booleanContains(buffer.geometry, geom));
    if (isCovered) covered++;
    else uncovered++;
  });

  const proposedCount = window._suggestedKmeansLayers
    ? window._suggestedKmeansLayers.filter(l => l instanceof L.Marker).length
    : 0;
...
document.getElementById("governorateDropdown").addEventListener("change", function () {
...
document.getElementById("closeDisplayBtn").addEventListener("click", function () {
